...
public class ACMCrawler : AbstractCrawler
{
    ...
    protected override string GetEffectiveUri()
    {
        ...
        return new Uri($"https://dl.acm.org/action/ajaxShowCitedBy?doi={doi}")
            .ToString();
    }
    ...
    protected override List<string> GetCitationsFromHtml(string html)
    {
        ...
        doc.LoadHtml(html);
        var citationNodes =
            doc.DocumentNode.SelectNodes("//li[@class='references__item']");

        var citations = citationNodes?.Select(node =>
        {
            // Try to find journal name in <span class="references__source">
            var sourceNode =
...
public abstract class AbstractCrawler(Uri uri) : ICrawler
{
    ...
    protected abstract string GetEffectiveUri();
